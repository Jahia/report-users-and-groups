...
@GraphQLName("ReportUsersAndGroupsQuery")
@GraphQLDescription("Report Users and Groups queries")
public class ReportUsersAndGroupsQuery {

    private static final Logger LOGGER = LoggerFactory.getLogger(ReportUsersAndGroupsQuery.class);
    ...
    public List<String> userProperties() {
        try {
            ...
            return Arrays.stream(userType.getPropertyDefinitions())
                    ...
                    .filter(name -> !name.startsWith("jcr:") && !name.startsWith("nt:") && !name.startsWith("rep:") && !name.equals("j:password") && !name.equals("*"))
                    .distinct()
                    .sorted()
    ...
    @GraphQLRequiresPermission("reportUsersAndGroupsAdmin")
    public List<GqlReportFile> listFiles(
